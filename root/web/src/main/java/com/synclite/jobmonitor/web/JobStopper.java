...
public class JobStopper implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			String jobRootDir = (String) jobDataMap.get("jobRootDir");
			String jobComponentName = (String) jobDataMap.get("jobComponentName");

			globalTracer.info("Scheduler attempting to stop job started by schedule : " + scheduleID);
			...
			while (line != null) {
				if (line.contains(jobComponentName) && line.contains(jobRootDir)) {
			...
			this.globalTracer.error("Failed to stop scheduled job : " + e.getMessage(), e);
		} finally {
			if (schedulerStatsPath != null) {
				try {
					//Make an entry in the schedule report.
					updateSchedulerStats(schedulerStatsPath, scheduleID, triggerId, jobStopTime, jobStopStatus, jobStopStatusDescription);
				} catch (Exception e) {
