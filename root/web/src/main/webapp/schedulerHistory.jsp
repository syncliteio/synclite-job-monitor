...
<title>SyncLite Job Scheduler History</title>
</head>
...
<body>
	...
	<div class="main">
		...
		<%
			...
			Path statsFilePath = Path.of(session.getAttribute("synclite-device-dir").toString(), "synclite_job_scheduler_statistics.db");
		...
		%>
			<center>
				<form name="tableForm" id="tableForm" method="post">
					...
					<tr>
						...
						<% 
							if (showScheduleDetails.equals("true")) {
								...
								out.println("<th>Run Interval(s)</th>");
								out.println("<th>Run Duration(s)</th>");
							}
						%>
						<th>Start Event</th>
						<th>Start Status</th>
					...
					<%
						String sql = "select schedule_id, job_name, job_type, job_sub_type, schedule_start_time, schedule_end_time, job_run_interval_s, job_run_duration_s, job_start_time, job_start_status, job_start_status_description, job_stop_time, job_stop_status, job_stop_status_description FROM statistics ORDER BY trigger_id DESC LIMIT " + numRecords;
						long idx = 1;
							...
							try (Statement stmt = conn.createStatement()) {
								try (ResultSet rs = stmt.executeQuery(sql)) {
									while (rs.next()) {
										...
										out.println("<td>" + rs.getString("schedule_id") + "</td>");
										out.println("<td>" + rs.getString("job_name") + "</td>");
										out.println("<td>" + rs.getString("job_type") + "</td>");
										out.println("<td>" + rs.getString("job_sub_type") + "</td>");
